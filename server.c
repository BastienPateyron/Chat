...
int main(){
    ...
    int		                socket_client[10];			        /* liste de 10 Sockets pour traiter un max de 10 clients */
    ...
    int                     num_client = 0;                    /* Numéro de 0 à 9 du client connecté */
    int                     nb_clients = 0;
    ...
    * S'il y en a un 4e, il sera rejete				*/
    ...
    while( 1 ){
        ...
        * juste pour lui. Au passage, on atrappe son adresse		*/
        socket_client[num_client] =	 accept(    socket_ecoute
            ,   (struct sockaddr *) &adresse_client
            ,   &taille);

            if ( socket_client[num_client] == -1 ){
                perror( "accept()" );
                exit( -1 );
            }
            else nb_clients++;
            ...
            printf(	    "Le client %d s'est connecté ... %s : %d\n"
                ...
                while(1){
                    ...
                    taille_recue = recv( socket_client[num_client], buffer_message, 128, 0);
                ...
                /* A ne pas oublier */
                close( socket_client[num_client] );

            }

            puts("Service terminé");
            close( socket_ecoute);
            return 0;
        }
